...
class Overview(MyView):
    def __init__(self, request):
        super(Overview, self).__init__(request, name='namewps', title='')
    ...
    def wps_services(self):
        ...
        for service in self.request.catalog.get_services(service_type=WPS_TYPE):
            try:
                ...
                LOGGER.debug('looking for: %s', self.request.registry.settings['namewps'])
                if service_name != self.request.registry.settings['namewps']:
                    continue
                url = self.request.route_path('namewps_list', _query=[('wps', service_name)])
    ...
    @view_config(route_name='namewps', renderer='../templates/overview.pt', accept='text/html')
    def view(self):
